...
class Movie(Base):
    """Movie database model - enhanced version."""
    __tablename__ = "movies"
    
    id = Column(Integer, primary_key=True, index=True)
    title = Column(String(255), nullable=False, index=True)
    year = Column(Integer, nullable=False, index=True)
    genre = Column(String(500), nullable=False, index=True)  # Pipe-separated genres
    director = Column(String(255), nullable=False)
    actors = Column(Text, nullable=False)  # Pipe-separated actors
...
class ApiKey(Base):
    """API Key database model - enhanced version."""
    __tablename__ = "api_keys"
    
    id = Column(Integer, primary_key=True, index=True)
    owner_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    key = Column(String(36), unique=True, nullable=False, index=True)  # UUID v4
    plan = Column(String(50), nullable=False, default="free")  # free, premium, etc.
    usage_count = Column(Integer, default=0)
    monthly_limit = Column(Integer, default=1000)  # Requests per month
...
class UsageLog(Base):
    """Usage logging database model - enhanced version."""
    ...
    ip_address = Column(String(45))  # IPv4 and IPv6 support
